...
class OrderManagement:
    ...
    def __place_order(self, type:OrderType,  volume:int, ticker:str):
        return self.avanza.place_order(
            ...
            order_type= type,
            price=self.get_latest_stock_price(ticker),
            valid_until=date.fromisoformat('%s'%(date.today().strftime("%Y-%m-%d"))),
            volume=volume) 



    def buy_order(self, stop_loss:float, profit_take:float, ticker:str, dt = datetime.datetime.now()):
    ...
    def sell_order(self,orderId, dt = datetime.datetime.now()):
      ...
      #  else: 
        ...
        sell_df = DataFrame([[dt, sell_price]],columns=["Datetime Sell","Sell"])
