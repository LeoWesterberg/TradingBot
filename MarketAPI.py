...
class MarketAPI:
    ...
    def get_data(self, ticker:str) -> DataFrame:
        data = yf.download(tickers = ticker, period = self.period, interval = self.interval,threads=True)
    ...
    def get_data_since(self, start:datetime, ticker:str) -> DataFrame:
        data:DataFrame = yf.download(tickers = ticker, start = start, interval = self.interval)
